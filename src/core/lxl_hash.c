...
int
lxl_hash1_init(lxl_hash1_t *hash, lxl_uint_t nelts, lxl_hash_key_pt key)
{	
	hash->buckets = lxl_alloc(nelts * sizeof(lxl_hash_elt_t *));
	if (hash->buckets == NULL) {
		return -1;
	}
...
int
lxl_hash1_add(lxl_hash1_t *hash, char *name, size_t len, void *value)
{
	lxl_uint_t hash_key;
	lxl_hash_elt_t *elt, *tmp, **curr, **buckets;
	...
	while (*curr) {
		tmp = *curr;
		if (tmp->len == len && memcmp(tmp->name, name, len) == 0) {
			//lxl_log_error(LXL_LOG_ERROR, 0, "hash add same key value");
			return -1;
		}
	...
	elt = (lxl_hash_elt_t *) lxl_alloc(sizeof(lxl_hash_elt_t) + len);
	if (elt == NULL) {
		return -1;
	}
...
void *
lxl_hash1_del(lxl_hash1_t *hash, char *name, size_t len)
{
	lxl_uint_t hash_key;
	lxl_hash_elt_t *tmp, **curr, **buckets;
...
void *
lxl_hash1_find(lxl_hash1_t *hash, char *name, size_t len)
{
	lxl_uint_t hash_key;
	lxl_hash_elt_t *tmp, **curr, **buckets;
...
void **
lxl_hash1_addfind(lxl_hash1_t *hash, char *name, size_t len)
{
	lxl_uint_t hash_key;
	lxl_hash_elt_t *elt, *tmp, **curr, **buckets;
