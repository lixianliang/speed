...
{
	lxl_log_init(LXL_LOG_DEBUG, LXL_LOG_DEBUG_ALL, LXL_LOG_FLUSH, NULL);
...
lxl_hash_sample(lxl_uint_t nelts)
{
	...
	for (i = 8; i < 18; ++i) {
		len = snprintf((char *) name, sizeof(name), "www.%lutaobao%lu.com", i, 40 - i);
		value = lxl_phash_del(&hash, name, len);
		if (value) {
	...
	for (i = 0; i < 40; ++i) {
		len = snprintf((char *) name, sizeof(name), "www.%lutaobao%lu.com", i, 40 - i);
		value = lxl_phash_find(&hash, name, len);
		if (value) {
